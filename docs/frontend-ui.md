...
小标题 “专注工作记录，AI 帮你写总结” (text-gray-600)
输入：用户名输入框 + 密码输入框
...
│ 最近更新：x分钟前（若有）                               ← 放在标题下方，text-sm gray-400 │
│                                                                             │
│ ┌─────────────────────────────────────────────────────────────────────────┐ │
│ │                         Tiptap 编辑器                                   │ │
│ │ 高度：calc(100vh - 220px)                                               │ │
...
- 今天页：标题下方增加“最近更新：x分钟前”提示（有日志时显示）。
- 报告页批量导出：在操作区保留单个导出按钮，若要批量导出，建议在列表上方补充“批量导出 PDF（全选/多选）”入口或在同一行提供“批量导出”按钮。
...
│ └──────────────────────┴──────────────────────────────────────────────────┘ │
│                                                                             │
│ 点击任一“编辑” → 弹出 Dialog（宽度约 600-700px）内嵌 Tiptap 编辑器          │
│ 弹窗底部：关闭 / 保存按钮，保存后更新预览并调用 /user/settings 接口        │
│ 初次加载：Skeleton 骨架占位                                                 │
└─────────────────────────────────────────────────────────────────────────────┘
```
数据流与状态：
- 进入页面：`GET /user/settings` 预填周报、月报模板。
- 保存：`PUT /user/settings` 仅更新模板字段（周报/ 月报），其他字段沿用当前值。
- 状态管理：`useSettingsStore` 负责加载、保存、toast 提示；页面用本地草稿控制弹窗与预览。
